...
export function BlockBasedEditor({
  ...
  // Template과 결혼식 정보가 제공된 경우 초기 블록 생성
  useEffect(() => {
    if (template && blocks.length === 0 && !invitationId) {
      // AIDEV-NOTE: 결혼식 정보를 기반으로 초기 블록 생성
...
// AIDEV-NOTE: 결혼식 정보를 기반으로 초기 블록 생성 (실제 데이터로 채움)
function createInitialBlocksFromWeddingInfo(weddingInfo: WeddingInfoFormData, template: Template): Block[] {
  const blocks: Block[] = [];
  let order = 0;
  ...
  blocks.push({
    ...
    data: {
      title: '결혼 인사',
      content: weddingInfo.customMessage || 
        '평생 서로 사랑하며 행복하게 살겠습니다.\n부디 오셔서 축복해 주시기 바랍니다.',
      isRichText: false,
    },
    styles: {
  ...
  if (template.category !== 'minimal') {
    blocks.push({
      ...
      isEditing: false,
      data: {
        imageUrl: weddingInfo.backgroundImageUrl || template.previewImageUrl || '',
        alt: '웨딩 사진',
        caption: '',
        aspectRatio: 'landscape',
      },
